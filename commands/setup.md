...
ghostink 把一个创作者拆成三件套 + 一个"个性化补丁",setup 全程都在这四件里打转。
**每一步要让用户做选择前,必须先用 1-3 行白话告诉 ta 这一步在选哪一件、为什么问、不选会怎样**——
不要直接抛 AskUserQuestion,使用者会懵。

| 名字 | 一句话定义 | 直观例子 |
| --- | --- | --- |
| **Soul** | 写什么 / 为什么写(价值观、关心什么、立场) | 王小波关心人的尊严与理性 |
| **Form** | 怎么写(句式、语气、节奏、用词) | 王小波的反讽 + 长句分析 |
| **Playbook** | 什么场景写什么(常用打法、发文节奏) | 猫笔刀的日报 + 热点评论 |
| **Profile** | 你自己的真实经历、观点、素材(写作时的"私货") | 你刚转行的故事、你独有的判断 |

> 一句话简记:Soul=写什么,Form=怎么写,Playbook=什么时候写哪种,Profile=你自己的料。
...
**先给用户一行背景**(然后再 AskUserQuestion):

> 在问读者画像之前,先大方向上分一下你写哪类内容——这决定我后面怎么问、给你
> 推荐哪些内置作家。
...
**先给用户一段背景**(plain text 输出,2-4 行,然后再调 AskUserQuestion):

> 这一步是选你要不要从已有作家那里"借"一个起点。ghostink 把作家拆成三件:
> **Soul**(写什么 / 为什么写)、**Form**(怎么写、句式语气)、**Playbook**(什么
> 场景写什么)。下面四个起步法,分别对应"三件全抄一个作家 / 三件混搭 / 自拆一个新
> 作家 / 完全不模仿"。新人推荐先全抄一个,跑顺了再混搭。

然后 AskUserQuestion 4 个分支:

> 你想从哪里起步搭建创作者档案?
>
> - A) 内置整套挂个作家 — Soul + Form + Playbook 全抄一个(推荐新人)
> - B) 多作家组合 — soul / form / playbook 分别从不同作家选
> - C) 自拆参考 — 有想拆的作者,先跑 library analyze 拆完再回来
...
**先给用户一段背景**(在 5.2 调 AskUserQuestion 之前输出):

> 这一步选 Form——决定**怎么写**:句子长短、用词偏好、语气节奏、有没有反讽。
> Soul 已经定了写什么,Form 定怎么写出来。两者可以同源(全抄一个作家的味道,
> 最稳),也可以混搭(比如借王小波的 Soul + 用三毛的 Form,出来是新物种,
> 但要留意兼容性警告)。
...
**先给用户一段背景**(在调 AskUserQuestion 之前输出):

> 这一步建 Playbook——你的"日常写作套路":发什么类型的文(日报 / 评论 / 教程 /
> 散文 / 长文 ...)、多长、什么场景下写什么。Soul + Form 决定一篇文章长什么样,
> Playbook 决定**什么时候写哪种**。可以先勾几个常用打法,以后用 write 时自然
> 会再加。
