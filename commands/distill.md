...
- 内置库里没有 ta,或现有 source 是 `built-in` / `ai-distilled`(精度低于 distilled)
...
| `--quick` | quick 模式(5+ 篇,~10 分钟,跳过 Phase 3 多轮迭代,source 标 `user-distilled-quick`) |
| `--deep` | deep 模式(默认,20+ 篇,30-60 分钟,完整 7 phase + 多轮迭代,source 标 `user-distilled-deep`) |
...
| 词频统计 | LLM 估算 | 跨所有文章统计 |
| 模板字段填满度 | ~60% | ≥90% |
| 输出 source | `user-distilled-quick` | `user-distilled-deep` |
| 用时 | ~10 min | 30-60 min |
...
写到 `[studio]/library/souls/<author>.md`,source 按模式
(`user-distilled-quick` 或 `user-distilled-deep`)。
...
deep 模式三件套写 `source: user-distilled-deep`。
quick 模式三件套写 `source: user-distilled-quick`。
...
- **quick 模式不是 deep 模式的子集**:quick 是阉割版,source 要醒目标
  `user-distilled-quick`,让用户知道这不是终态
