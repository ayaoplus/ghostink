...
| 版本 | 含义 |
|---|---|
| `1.0-draft` | AI 直接产出的初版,无样本验证,UI 显示 ⚠ |
| `1.0-quick` | 用 5+ 篇文章跑 `distill --quick` 拆出(跳过多轮迭代),UI 显示 ⚠ |
| `2.0-distilled` | 用 20+ 篇代表作跑 `distill`(deep 模式)拆出,经多轮 predict-then-compare 收敛,推荐使用 |

`source` 字段对应:
- `built-in`:仓库自带
- `ai-distilled`:AI 训练知识 + 抽样产出(setup 流程默认)
- `user-distilled-quick`:用户跑 `/ghostink distill --quick` 拆出
- `user-distilled-deep`:用户跑 `/ghostink distill`(默认 deep)拆出
...
4. version 字段为 `2.0-distilled`(基于 20+ 篇代表作走 `/ghostink distill` 完整拆出)
