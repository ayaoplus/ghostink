...
3. /ghostink library          换文笔、看库存
4. /ghostink distill <author> 从历史文章提炼一位作者(可选,深度自定义)
5. /ghostink deai [file]      任意文本去 AI 感(独立扫描器,不依赖 form)
```

5 个一级命令。新人最常用 setup + write 两步起步,其他按需。
...
加 `--quick` 走 5+ 篇 ~10 分钟轻量版。这是 ghostink 的核心提炼引擎,
内置库 maobidao 套件就是用这个流程产出。详见 `commands/distill.md`。

### `/ghostink deai [file]`
任意中文文本去 AI 感扫描 + 修订建议。**不依赖任何作家 form**,通用 50+
规则覆盖 4 大类(零容忍清单 / 句式与结构 / 词频与措辞 / 内容深度)+ 1 个
活人感终审。每条 FAIL 都带具体替换方案。融合维基 24 种 AI 写作痕迹 +
卡兹克禁用词 + 内部规则。详见 `commands/deai.md`。
...
| 子命令 | 用途 |
|---|---|
| `library list` | 列出内置 + 用户自拆的 soul/form/playbook,标 source 标记 |
...
| 命令 | 用途 |
|---|---|
| `/ghostink check [file]` | 任意文本审查(form 合规,**需当前 studio 有 form.md**) |
| `/ghostink illustrate [file]` | 配图(挂载式,调外部图像生成 CLI) |

> `check` 跟 `deai` 互补:**check 看像不像作家,deai 看像不像 AI**。两者
> 跑全更稳。
...
| `/ghostink distill` | `commands/distill.md` |
| `/ghostink deai` | `commands/deai.md`(独立一级命令,完整 spec 在此) |
